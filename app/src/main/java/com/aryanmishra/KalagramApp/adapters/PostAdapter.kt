...
import com.aryanmishra.KalagramApp.databinding.PostRvBinding
...
class PostAdapter(var context: Context, var postList: ArrayList<Post>) : RecyclerView.Adapter<PostAdapter.MyHolder>() {
    ...
    override fun onBindViewHolder(holder: MyHolder, position: Int) {
        ...
        try {
            Firebase.firestore.collection(USER_NODES).document(postList.get(position).uid).get().addOnSuccessListener {
        ...
        holder.binding.like.setOnClickListener {
            holder.binding.like.setImageResource(R.drawable.heart3)
        }
